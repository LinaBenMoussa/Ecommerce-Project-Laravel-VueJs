<template>
    <div class="col-md-6 offset-md-3 border rounded p-4 mt-2
        ...
        <form @submit.prevent="modifierproduit">
            ...
            <router-link to="/listarticle" class="btn btn-outline-danger
...
const fetchArticle = async () => {
    await axios.get(`http://localhost:8000/api/articles/${route.params.id}`)
    .then((res) => {
...
const serverOptions = () => {
    ...
    return {
        process: (fieldName, file, metadata, load, error, progress, abort) => {
            ...
            data.append('upload_preset', 'gliid5');
